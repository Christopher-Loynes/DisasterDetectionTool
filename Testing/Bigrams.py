...
svm = OneVsRestClassifier(LinearSVC(C=1, random_state=0))
nb = OneVsRestClassifier(MultinomialNB(alpha=1))
lr = OneVsRestClassifier(LogisticRegression(C=1, random_state=0))
rf = OneVsRestClassifier(RandomForestClassifier(n_estimators=200,criterion="gini",
...
cross_val_predict_svm = cross_val_predict(svm,x_bigrams, y_label, cv=10)
cross_val_predict_nb = cross_val_predict(nb,x_bigrams, y_label, cv=10)
cross_val_predict_lr = cross_val_predict(lr,x_bigrams, y_label, cv=10)
cross_val_predict_rf = cross_val_predict(rf,x_bigrams, y_label, cv=10)
...
filename = "accuracies_all_models_bigrams.csv"
...
for cross_pred in cross_val_list:
    ...
    plt.title('ROC For ' + str(cross_val_names[b]) + '_bigrams')
    plt.legend(loc="lower right")
    plt.savefig('ROC_' + str(cross_val_names[b]) + '_bigrams.png')
    plt.show()
    plt.close()
