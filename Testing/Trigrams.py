...
########## Fitting, predicting and calculating average accuracy for trigrams data ##########
...
cross_val_predict_svm = cross_val_predict(svm,x_trigrams, y_label, cv=10)
cross_val_predict_nb = cross_val_predict(nb,x_trigrams, y_label, cv=10)
cross_val_predict_lr = cross_val_predict(lr,x_trigrams, y_label, cv=10)
cross_val_predict_rf = cross_val_predict(rf,x_trigrams, y_label, cv=10)
...
filename = "accuracies_all_models_trigrams.csv"
...
for cross_pred in cross_val_list:
    ...
    plt.title('ROC For ' + str(cross_val_names[b]) + '_trigrams')
    plt.legend(loc="lower right")
    plt.savefig('ROC_' + str(cross_val_names[b]) + '_trigrams.png')
    plt.show()
    plt.close()
